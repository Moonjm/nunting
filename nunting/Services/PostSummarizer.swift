...
nonisolated enum PostSummaryPrompt {
    ...
    ///
    /// **반응 지시는 여기 두지 않는다.** 예전엔 "댓글이 있으면 반응을
    /// 덧붙이라"를 여기 뒀는데, 소형 온디바이스 모델이 조건절("있으면")만
    /// 흘리고 반응 한 줄을 **항상 출력 템플릿**으로 각인해, 댓글이 없어도
    /// "네티즌 반응은…"을 지어냈다. 조건이 물리적으로 성립할 때(=프롬프트에
    /// 베스트 댓글이 실릴 때)만 존재하도록 반응 지시는 프롬프트 분기로 옮긴다.
    static let instructions = """
    당신은 한국 커뮤니티 글을 요약하는 도우미입니다. 주어진 글을 한국어로 \
    핵심만 2~4문장으로 간결하게 요약하세요. 과장하거나 없는 내용을 \
    지어내지 마세요.
    """
    ...
    static func build(detail: PostDetail) -> String {
        ...
        // 댓글이 없으면 반응을 **언급조차 하지 않는다** — 부정문("쓰지
        // 마세요")조차 소형 모델엔 재각인이라, 반응 지시는 댓글이 실제로
        // 실릴 때만 프롬프트에 넣는다(instructions 주석 참조).
        if !top.isEmpty {
            prompt += "\n\n베스트 댓글:"
            for c in top {
                prompt += "\n- (공감 \(c.likeCount)) \(String(c.content.prefix(maxCommentChars)))"
            }
            prompt += "\n\n위 베스트 댓글을 바탕으로 전체적인 반응을 한 문장으로 요약 끝에 덧붙이세요."
        }
        return prompt
    }
