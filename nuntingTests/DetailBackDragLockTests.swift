...
final class DetailBackDragLockTests: XCTestCase {
    ...
    /// 를 안 부를 수 있다. 그러면 드래그가 켠 것들이 켜진 채로 남는데, 그중
    /// 가장 아픈 것이 스크롤 잠금이다 — 안 풀리면 상세 스크롤이 죽은 채로
    /// 남는다. 취소도 손을 뗀 것과 같게 정착 스프링을 거쳐 풀려야 한다.
    @MainActor
    func testCancellingADragEventuallyReleasesTheScrollLock() {
        let drag = DetailBackDrag()
        defer { DetailScrollLock.shared.isLocked = false }
